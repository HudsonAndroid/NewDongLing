...
public class RecyclerLyricsView extends RecyclerView implements ILyricsView {
    public static final int DEFAULT_FOCUS_LYRICS_COLOR = Color.RED;
    public static final int DEFAULT_NORMAL_LYRICS_COLOR = Color.LTGRAY;
    ...
    public int setLyricsCount(int count){
        ...
        count = Math.max(count, MIN_LYRICS_COUNT);
        count = Math.min(count, MAX_LYRICS_COUNT);
    ...
    public void setLyrics(@NonNull List<Lyrics> lyrics,@NonNull List<Long> timeList, long startTime) {
        if(lyrics.size() == timeList.size()){
            ...
            throw new IllegalArgumentException("the lyrics list size doesn't match time list");
        }
    }
...
//        Log.e("hudson","需要滑动到的位置"+(position+mAdapter.getLyricsIndexOffset()));
    ...
    public boolean isPlaying(){
        return mLyricsSchedule.isWorkRunning();
    }
